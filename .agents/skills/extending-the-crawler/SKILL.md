...
description: How to extend the crawler following its conventions - add a newsletter source (the add command / UI wizard now persists to config/sources.json), add a new src/ module, add a CLI command, register a publication, or add a new fetch/extract strategy. Use whenever you add a source, feature, module, command, or new capability to the crawler.
metadata:
  type: task
...
## Procedure
- **Add a source:** `npm run add -- <url> --name "..." [--type index|listing] [--max-index-pages N]`, or the UI `Adicionar fonte` wizard, or hand-edit `config/sources.json`. `add`/the wizard now PERSIST to `config/sources.json` via `addSourceToConfig` (upsert by normalized URL) AND enqueue the seed — so the source is permanent: it shows in the UI Coletar picker (which reads `loadSources()`) and is re-seeded every crawl. `type:'index'` means the source's links are issues/roundups; default `listing` means links are articles. `src/config.js@fc0e1be`, `src/commands.js@fc0e1be`. Then run a bounded crawl to confirm links are found and a selector is derived/cached.
