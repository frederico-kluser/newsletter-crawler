...
- **All SQL via one `stmts` object.** `claimNext` is an atomic `UPDATE frontier SET state='in_progress' ... RETURNING *` (hands out one job at a time, safe under the async loop); `enqueue` is `INSERT OR IGNORE` (URL dedup in the queue); upserts use `ON CONFLICT`. `src/db.js:68-132@79fd5d8`.
...
- **Concurrency loop.** `pLimit(CONCURRENCY)` plus an `inflight` Set plus `Promise.race`; the loop stops when `inflight` is empty and `claimNext` returns nothing. `--max-articles` caps further claiming. `src/index.js:64-92@79fd5d8`.
- **Pagination (`crawlArchive`) stop conditions:** an empty page (selector yields under the min links), a repeated `html_hash`, or no next link. `findNextPage` order: cached `next_selector` -> `<a rel="next">` -> `?page=N+1` -> Flash LLM (which caches the discovered `next_selector`). `src/crawl.js:117-189@79fd5d8`.
