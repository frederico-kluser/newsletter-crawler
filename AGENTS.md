...
- pipeline de qualidade (default ON): issues de fontes `index` são CURADAS por IA em itens `kind` news|tool|release com o blurb do agregador (cadastro na curadoria; o corpo do alvo vira enriquecimento — `needs_enrich`, e alvo raso/bloqueado NÃO perde o item); conteúdo extraído passa por limpeza IA pré-save (`sanityCheckCleaned` anti-truncamento); pós-crawl roda verificação (ok|suspect|junk por artigo). Desliga: `CURATE_ROUNDUPS`/`CLEAN_BEFORE_SAVE`/`VERIFY_AFTER_CRAWL=false`, `--no-verify`.
