...
import { clampFutureDate, parseDate } from '../src/util.js'; // util é PURO (não importa config)
...
process.env.NC_HOME = mkdtempSync(path.join(os.tmpdir(), 'nc-clean-date-'));
after(() => rmSync(process.env.NC_HOME, { recursive: true, force: true }));
const { extractPublishedDate } = await import('../src/clean.js');
